...
/**
 * Destructive confirmations (TXE-21928): pending values and “show dialog” flags live on the fact graph
 * (/flowPending*, /flowShow* in flowConfirmations.xml). fg-set-before-commit sets them and blocks commit;
 * modal actions clear or apply them.
 */
...
function resetEntireGraphKeepingTaxYear (newYearValue) {
  sessionStorage.removeItem('factGraph')
  factGraph = fg.GraphFactory.apply(factDictionary)
  window.factGraph = factGraph
...
function refreshNavigationLinks (manifest) {
...
async function initSingleQuestionNav () {
