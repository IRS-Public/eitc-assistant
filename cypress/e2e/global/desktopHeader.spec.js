/* eslint-disable newline-per-chained-call */
/* eslint-disable max-len */

const searchText = '1040'

describe('Global Test', () => {
    it('Header', () => {
        cy.get('@contentJSON').then((content) => {
            cy.viewport(1280, 1024)
            cy.get('[data-testid="U.S. Flag"]').should('be.visible')
            cy.get('[data-testid="US Flag text"]').should('have.text', content.global.header.govWebsiteText).and('be.visible')
            cy.get('[data-testid="irsgovLink"]').should('have.text', content.global.header.irsLogo).and('have.attr', 'href', content.global.headerLink.irsLogoLink).and('be.visible')
            cy.get('[data-testid="IRSLogo"]').should('be.visible')
            cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.header.help).and('have.attr', 'href', content.global.headerLink.helpLink).and('be.visible')
            cy.get('[data-testid=newsLinkDesktop]').should('have.text', content.global.header.news).and('have.attr', 'href', content.global.headerLink.newsLink).and('be.visible')
            cy.get('[data-testid="charitiesLinkDesktop"]').should('have.text', content.global.header.charities).and('have.attr', 'href', content.global.headerLink.charitiesLink).and('be.visible')
            cy.get('[data-icon="hands-helping"]').should('be.visible')
            cy.get('[data-testid="taxProsLinkDesktop"]').should('have.text', content.global.header.taxPros).and('have.attr', 'href', content.global.headerLink.taxProsLink).and('be.visible')
            cy.get('[data-icon="book"]').should('be.visible')
            cy.get('[data-testid="searchBoxDesktop"]').type(searchText).should('have.value', searchText).and('be.visible')
            cy.get('#navBarSearchForm').should('have.attr', 'target', '_blank').and('have.attr', 'action', content.global.headerLink.searchLink).and('have.attr', 'method', 'GET')
            
            // File megamenu
            cy.get('[data-testid="fileLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 1;')
            cy.get('[data-testid="fileLink"]').scrollIntoView().should('have.text', content.global.header.file).and('have.attr', 'href', content.global.headerLink.fileLink).and('be.visible')
            cy.get('[data-testid="infoForTextFile"]').should('have.text', content.global.header.informationFor).and('be.visible')
            cy.get('[data-testid="individualsLink"]').should('have.text', content.global.header.individuals).and('have.attr', 'href', content.global.headerLink.individualsLink).and('be.visible') 
            cy.get('[data-testid="businessLink"]').should('have.text', content.global.header.businessSelfEmployed).and('have.attr', 'href', content.global.headerLink.businessSelfEmployedLink).and('be.visible')
            cy.get('[data-testid="charitiesLink"]').should('have.text', content.global.header.charities2).and('have.attr', 'href', content.global.headerLink.charities2Link).and('be.visible')
            cy.get('[data-testid="internationalLink"]').should('have.text', content.global.header.internationalTaxpayers).and('have.attr', 'href', content.global.headerLink.internationalTaxpayersLink).and('be.visible')
            cy.get('[data-testid="govLink"]').should('have.text', content.global.header.stateLocal).and('have.attr', 'href', content.global.headerLink.stateLocalLink).and('be.visible')
            cy.get('[data-testid="tribalLink"]').should('have.text', content.global.header.indianTribal).and('have.attr', 'href', content.global.headerLink.indianTribalLink).and('be.visible')
            cy.get('[data-testid="bondsLink"]').should('have.text', content.global.header.bonds).and('have.attr', 'href', content.global.headerLink.bondsLink).and('be.visible')
            cy.get('[data-testid="filingForIndvText"]').should('have.text', content.global.header.filingForIndividuals).and('be.visible')
            cy.get('[data-testid="howToFileLink"]').should('have.text', content.global.header.howToFile).and('have.attr', 'href', content.global.headerLink.howToFileLink).and('be.visible')
            cy.get('[data-testid="whenToFileLink"]').should('have.text', content.global.header.whenToFile).and('have.attr', 'href', content.global.headerLink.whenToFileLink).and('be.visible')
            cy.get('[data-testid="whereToFileLink"]').should('have.text', content.global.header.whereToFile).and('have.attr', 'href', content.global.headerLink.whereToFileLink).and('be.visible')
            cy.get('[data-testid="updateMyInfoLink"]').should('have.text', content.global.header.updateInformation).and('have.attr', 'href', content.global.headerLink.updateInformationLink).and('be.visible')       
            cy.get('[data-testid="popularTextFile"]').should('have.text', content.global.header.popular).and('be.visible')
            cy.get('[data-testid="economicImpactLink"]').should('have.text', content.global.header.economicImpact).and('have.attr', 'href', content.global.headerLink.economicImpactLink).and('be.visible')
            cy.get('[data-testid="coronavirusLink"]').should('have.text', content.global.header.coronavirus).and('have.attr', 'href', content.global.headerLink.coronavirusLink).and('be.visible')
            cy.get('[data-testid="taxRecordLink"]').should('have.text', content.global.header.taxRecord).and('have.attr', 'href', content.global.headerLink.taxRecordLink).and('be.visible')
            cy.get('[data-testid="ipPinLink"]').should('have.text', content.global.header.IPPIN).and('have.attr', 'href', content.global.headerLink.IPPINLink).and('be.visible')
            cy.get('[data-testid="fileLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 0;')

            // Pay megamenu
            cy.get('[data-testid="payLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 1;')
            cy.get('[data-testid="payLink"]').scrollIntoView().should('have.text', content.global.header.pay).and('have.attr', 'href', content.global.headerLink.payLink).and('be.visible')
            cy.get('[data-testid="payByText"]').should('have.text', content.global.header.payBy).and('be.visible')
            cy.get('[data-testid="bankAcctLink"]').should('have.text', content.global.header.backAccount).and('have.attr', 'href', content.global.headerLink.backAccountLink).and('be.visible')
            cy.get('[data-testid="debitCCLink"]').should('have.text', content.global.header.debitOrCreditCard).and('have.attr', 'href', content.global.headerLink.debitOrCreditCardLink).and('be.visible')
            cy.get('[data-testid="paymenPlanLink"]').should('have.text', content.global.header.paymentPlan).and('have.attr', 'href', content.global.headerLink.paymentPlanLink).and('be.visible')
            cy.get('[data-testid="eftpsLink"]').should('have.text', content.global.header.eftps).and('have.attr', 'href', content.global.headerLink.eftpsLink).and('be.visible')
            cy.get('[data-testid="popularTextPay"]').should('have.text', content.global.header.popular).and('be.visible')
            cy.get('[data-testid="viewyourAcctLink"]').should('have.text', content.global.header.viewYourAccount).and('have.attr', 'href', content.global.headerLink.viewYourAccountLink).and('be.visible')
            cy.get('[data-testid="tweLink"]').should('have.text', content.global.header.twe).and('have.attr', 'href', content.global.headerLink.tweLink).and('be.visible')
            cy.get('[data-testid="estimatedTaxesLink"]').should('have.text', content.global.header.estimatedTaxes).and('have.attr', 'href', content.global.headerLink.estimatedTaxesLink).and('be.visible')
            cy.get('[data-testid="penaltiesLink"]').should('have.text', content.global.header.penalties).and('have.attr', 'href', content.global.headerLink.penaltiesLink).and('be.visible')
            cy.get('[data-testid="payLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 0;')
            
            // Refunds megamenu
            cy.get('[data-testid="refundLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 1;')
            cy.get('[data-testid="refundLink"]').scrollIntoView().should('have.text', content.global.header.refunds).and('have.attr', 'href', content.global.headerLink.refundsLink)
            cy.get('[data-testid="whereMyRefundLink"]').should('have.text', content.global.header.wheresMyRefund).and('have.attr', 'href', content.global.headerLink.wheresMyRefundLink).and('be.visible')
            cy.get('[data-testid="whatToExpectLink"]').should('have.text', content.global.header.whatToExpect).and('have.attr', 'href', content.global.headerLink.whatToExpectLink).and('be.visible')
            cy.get('[data-testid="directDepositLink"]').should('have.text', content.global.header.directDeposit).and('have.attr', 'href', content.global.headerLink.directDepositLink).and('be.visible')
            cy.get('[data-testid="reducedRefundsLink"]').should('have.text', content.global.header.reducedRefunds).and('have.attr', 'href', content.global.headerLink.reducedRefundsLink).and('be.visible')
            cy.get('[data-testid="fixReturnLink"]').should('have.text', content.global.header.fixCorrectAReturn).and('have.attr', 'href', content.global.headerLink.fixCorrectAReturnLink).and('be.visible')
            cy.get('[data-testid="refundLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 0;')
            
            // Credits & Deductions megamenu
            cy.get('[data-testid="creditsDeductionsLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 1;')
            cy.get('[data-testid="creditsDeductionsLink"]').scrollIntoView().should('have.text', content.global.header.credits).and('have.attr', 'href', content.global.headerLink.creditsLink)
            cy.get('[data-testid="infoForTextCD"]').should('have.text', content.global.header.informationCredits).and('be.visible')
            cy.get('[data-testid="youFamilyLink"]').should('have.text', content.global.header.individualsCredits).and('have.attr', 'href', content.global.headerLink.individualsCreditsLink).and('be.visible')
            cy.get('[data-testid="youFamilyLinkText"]').should('have.text', content.global.header.individualsCreditsText).and('be.visible')
            cy.get('[data-testid="businessCredLink"]').should('have.text', content.global.header.businessSelfEmployedCredits).and('have.attr', 'href', content.global.headerLink.businessSelfEmployedCreditsLink).and('be.visible')
            cy.get('[data-testid="businessCredLinkText"]').should('have.text', content.global.header.businessSelfEmployedCreditsText).and('be.visible')
            cy.get('[data-testid="poularTextCD"]').should('have.text', content.global.header.popular).and('be.visible')
            cy.get('[data-testid="eitcLink"]').should('have.text', content.global.header.eitc).and('have.attr', 'href', content.global.headerLink.eitcLink).and('be.visible')
            cy.get('[data-testid="ctcLink"]').should('have.text', content.global.header.childTaxCredit).and('have.attr', 'href', content.global.headerLink.childTaxCreditLink).and('be.visible')
            cy.get('[data-testid="standardDedLink"]').should('have.text', content.global.header.standardDeduction).and('have.attr', 'href', content.global.headerLink.standardDeductionLink).and('be.visible')
            cy.get('[data-testid="healthCovgLink"]').should('have.text', content.global.header.healthCoverage).and('have.attr', 'href', content.global.headerLink.healthCoverageLink).and('be.visible')
            cy.get('[data-testid="retirementLink"]').should('have.text', content.global.header.retirementSavings).and('have.attr', 'href', content.global.headerLink.retirementSavingsLink).and('be.visible')
            cy.get('[data-testid="creditsDeductionsLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 0;')
            
            // Forms megamenu
            cy.get('[data-testid="formsInsLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 1;')
            cy.get('[data-testid="formsInsLink"]').scrollIntoView().should('have.text', content.global.header.forms).and('have.attr', 'href', content.global.headerLink.formsLink)
            cy.get('[data-testid="formsInsText"]').should('have.text', content.global.header.formsAndInstructions).and('be.visible')
            cy.get('[data-testid="1040Link"]').should('have.text', content.global.header.form1040).and('have.attr', 'href', content.global.headerLink.form1040Link).and('be.visible')
            cy.get('[data-testid="1040LinkText"]').should('have.text', content.global.header.form1040Text).and('be.visible')
            cy.get('[data-testid="1040iLink"]').should('have.text', content.global.header.form1040Instructions).and('have.attr', 'href', content.global.headerLink.form1040InstructionsLink).and('be.visible')
            cy.get('[data-testid="1040iLinkText"]').should('have.text', content.global.header.form1040InstructionsText).and('be.visible')
            cy.get('[data-testid="w9Link"]').should('have.text', content.global.header.formW9).and('have.attr', 'href', content.global.headerLink.formW9Link).and('be.visible')
            cy.get('[data-testid="w9LinkText"]').should('have.text', content.global.header.formW9Text).and('be.visible')
            cy.get('[data-testid="4506tLink"]').should('have.text', content.global.header.form4506T).and('have.attr', 'href', content.global.headerLink.form4506TLink).and('be.visible')
            cy.get('[data-testid="4506tLinkText"]').should('have.text', content.global.header.form4506TText).and('be.visible')
            cy.get('[data-testid="w4Link"]').should('have.text', content.global.header.formW4).and('have.attr', 'href', content.global.headerLink.formW4Link).and('be.visible')
            cy.get('[data-testid="w4LinkText"]').should('have.text', content.global.header.formW4Text).and('be.visible')
            cy.get('[data-testid="941Link"]').should('have.text', content.global.header.form941).and('have.attr', 'href', content.global.headerLink.form941Link).and('be.visible')
            cy.get('[data-testid="941LinkText"]').should('have.text', content.global.header.form941Text).and('be.visible')
            cy.get('[data-testid="w2Link"]').should('have.text', content.global.header.formW2).and('have.attr', 'href', content.global.headerLink.formW2Link).and('be.visible')
            cy.get('[data-testid="w2LinkText"]').should('have.text', content.global.header.formW2Text).and('be.visible')
            cy.get('[data-testid="9465Link"]').should('have.text', content.global.header.form9465).and('have.attr', 'href', content.global.headerLink.form9465Link).and('be.visible')
            cy.get('[data-testid="9465LinkText"]').should('have.text', content.global.header.form9465Text).and('be.visible')
            cy.get('[data-testid="popularTaxProsText"]').should('have.text', content.global.header.popularForms).and('be.visible')
            cy.get('[data-testid="1040xLink"]').should('have.text', content.global.header.form1040X).and('have.attr', 'href', content.global.headerLink.form1040XLink).and('be.visible')
            cy.get('[data-testid="1040xLinkText"]').should('have.text', content.global.header.form1040XText).and('be.visible')
            cy.get('[data-testid="2848Link"]').should('have.text', content.global.header.form2848).and('have.attr', 'href', content.global.headerLink.form2848Link).and('be.visible')
            cy.get('[data-testid="2848LinkText"]').should('have.text', content.global.header.form2848Text).and('be.visible')
            cy.get('[data-testid="w7Link"]').should('have.text', content.global.header.formW7).and('have.attr', 'href', content.global.headerLink.formW7Link).and('be.visible')
            cy.get('[data-testid="w7LinkText"]').should('have.text', content.global.header.formW7Text).and('be.visible')
            cy.get('[data-testid="230Link"]').should('have.text', content.global.header.circular230).and('have.attr', 'href', content.global.headerLink.circular230Link).and('be.visible')
            cy.get('[data-testid="230LinkText"]').should('have.text', content.global.header.circular230Text).and('be.visible')
            cy.get('[data-testid="formsInsLink"] + div').invoke('attr', 'style', 'visibility: visible;opacity: 0;')
        })
    })
    // it('Language Switcher', () => {
    //     cy.get('@contentJSON').then((content) => {
    //         cy.viewport(1280, 1024)
    //              // When app is already in Spanish, switch back to English first so that the Spanish link is visible in the lang switcher
    //              cy.url().then(($url) => {
    //                 if ($url.includes('/es')) {
    //                     cy.get('#languageButton').should('be.visible').click()
    //                 cy.get('[data-testid="englishLinkDesktop"]').last().click()
    //                 }
    //             })
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="spanishLinkDesktop"]').first().should('have.text', content.global.languageSwitcher.spanish).and('have.attr', 'href', content.global.languageSwitcher.spanishLink).and('be.visible').click()
    //             cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.languageSwitcher.spanishHelpLink).and('be.visible')
    //             cy.get('[data-testid="generalInfoHeader"]').should('have.text', content.global.languageSwitcher.spanishTitle).and('be.visible')
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="chinese1LinkDesktop"]').first().should('have.text', content.global.languageSwitcher.chinese1).and('have.attr', 'href', content.global.languageSwitcher.chinese1Link).and('be.visible').click()
    //             cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.languageSwitcher.chinese1HelpLink).and('be.visible')
    //             cy.get('[data-testid="generalInfoHeader"]').should('have.text', content.global.languageSwitcher.chinese1Title).and('be.visible')
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="chinese2LinkDesktop"]').first().should('have.text', content.global.languageSwitcher.chinese2).and('have.attr', 'href', content.global.languageSwitcher.chinese2Link).and('be.visible').click()
    //             cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.languageSwitcher.chinese2HelpLink).and('be.visible')
    //             cy.get('[data-testid="generalInfoHeader"]').should('have.text', content.global.languageSwitcher.chinese2Title).and('be.visible')
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="koreanLinkDesktop"]').first().should('have.text', content.global.languageSwitcher.korean).and('have.attr', 'href', content.global.languageSwitcher.koreanLink).and('be.visible').click()
    //             cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.languageSwitcher.koreanHelpLink).and('be.visible')
    //             cy.get('[data-testid="generalInfoHeader"]').should('have.text', content.global.languageSwitcher.koreanTitle).and('be.visible')
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="russianLinkDesktop"]').first().should('have.text', content.global.languageSwitcher.russian).and('have.attr', 'href', content.global.languageSwitcher.russianLink).and('be.visible').click()
    //             cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.languageSwitcher.russianHelpLink).and('be.visible')
    //             cy.get('[data-testid="generalInfoHeader"]').should('have.text', content.global.languageSwitcher.russianTitle).and('be.visible')
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="vietnameseLinkDesktop"]').first().should('have.text', content.global.languageSwitcher.vietnamese).and('have.attr', 'href', content.global.languageSwitcher.vietnameseLink).and('be.visible').click()
    //             cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.languageSwitcher.vietnameseHelpLink).and('be.visible')
    //             cy.get('[data-testid="generalInfoHeader"]').should('have.text', content.global.languageSwitcher.vietnameseTitle).and('be.visible')
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="haitianLinkDesktop"]').first().should('have.text', content.global.languageSwitcher.haitian).and('have.attr', 'href', content.global.languageSwitcher.haitianLink).and('be.visible').click()
    //             cy.get('[data-testid="helpLinkDesktop"]').should('have.text', content.global.languageSwitcher.haitianHelpLink).and('be.visible')
    //             cy.get('[data-testid="generalInfoHeader"]').should('have.text', content.global.languageSwitcher.haitianTitle).and('be.visible')
    //             cy.get('#languageButton').should('be.visible').click()
    //             cy.get('[data-testid="englishLinkDesktop"]').first().should('have.text', content.global.languageSwitcher.english).and('have.attr', 'href', content.global.languageSwitcher.englishLink).and('be.visible').click()
    //   })
   // })
})
